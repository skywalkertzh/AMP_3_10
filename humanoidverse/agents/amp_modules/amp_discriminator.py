...
class AMPDiscriminator(nn.Module):
    ...
    def predict_amp_reward(
        ...
        #     #
        #     amp_reward = torch.clamp(1 - (1/4) * torch.square(d - 1), min=0)
        #     #
        ...
        with torch.no_grad():
            ...
            if normalizer is not None:
                state = normalizer.normalize_torch(state, self.device)
                next_state = normalizer.normalize_torch(next_state, self.device)
            d = self.amp_linear(self.trunk(torch.cat([state, next_state], dim=-1))) # discriminator
        ...
        return reward.squeeze(), d, style_reward.squeeze()
    ...
    def discrimination_test(self, state, next_state):
        with torch.no_grad():
            self.eval()
            d = self.amp_linear(self.trunk(torch.cat([state, next_state], dim=-1)))
            self.train()
    ...
    def discrimination_test_reward(self, state, next_state):
        d = self.discrimination_test(state, next_state)
        return self.amp_reward_coef * torch.clamp(1 - (1/4) * torch.square(d - 1), min=0)
    
    def predict_amp_reward_test(self, state, next_state, normalizer=None):
        with torch.no_grad():
            self.eval()
            if normalizer is not None:
                state = normalizer.normalize_torch(state, self.device)
